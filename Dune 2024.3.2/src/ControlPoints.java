...
public class ControlPoints implements PaintUpdatable{
	...
	public boolean isThereNode(int row, int col) {
		Node n = new Node(col,row,null,0,0);
	...
	public void delete(int row, int col,Unit u) {
		Node n = new Node(col,row,null,0,0);
		if(list.size()==1) {
			list.set(0, u.getCurrentNode());
			return;
		}
		list.remove(n);
		if(currentIndex>list.indexOf(n)) {
			currentIndex--;
		}
		else if(currentIndex == list.indexOf(n)&&currentIndex!=0){
			currentIndex++;
		}
	}
	public boolean nextNodeByType(int typeOfBlock) {
		StrategyType t = type;
		type  = StrategyType.CIRCLE;
		boolean found = false;
	for(int i = 0; i < list.size();i++)	{
		nextNode();
		Node n = list.get(currentIndex);
		if(Map.getBlock(n.y, n.x).getType() == typeOfBlock) {
			found = true;
			break;
		}
	}
	...
	public void paint(Graphics g) {
		...
		if(type == StrategyType.LAST) {
			Node last = list.get(currentIndex);
			g.drawRect(
