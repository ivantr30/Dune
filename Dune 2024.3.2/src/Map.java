...
public class Map {
	...
	private static Color selected = new Color(76,255,0,50);
	...
	public static void paint(Graphics g) {
		...
		for (int row = firstRow; row <= lastRow; row++) {
			for (int col = firstCol; col <= lastCol; col++) {
				...
				if (ContextManager.isNodeActive()&&ContextManager.getCurrentNode().y == row && ContextManager.getCurrentNode().x ==col) {
					g.setColor(selected);
					g.fillRect(
							Camera.getScreenX(col * Block.SIZE),
							Camera.getScreenY(row * Block.SIZE), 
