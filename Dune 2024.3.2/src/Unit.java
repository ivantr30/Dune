...
public abstract class Unit implements PaintUpdatable{
	...
	public Unit(int x, int y, double speed, int mainPicId) {
		...
		main.setActive(true);
		if(bot) {
			//points = new ControlPoints((int)(Math.random()*50),(int)(Math.random()*50));
			points = new ControlPoints(3,5);
//			points.add(3, 30);
//			points.add(20, 30);
//			points.add(20, 10);
			findNewTarget();
	...
	public void showOnStatusBar() {
		StatusBar.show(getTextForStatusBar(),5000);
	}
	...
	public void deleteControlPoint(int row, int col) {
		points.delete(row, col,this);
	}
	...
	public boolean isThereNode(int row, int col) {
		return points.isThereNode(row, col);
	}
...
public void paint(Graphics g) {
		...
		if(selected) {
			...
			g.drawRect(Camera.getScreenX(getX()), Camera.getScreenY(getY()), Block.SIZE, Block.SIZE);
			points.paint(g);
		}
	}
