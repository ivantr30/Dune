...
public class Map {
	...
	private static Point selectedCell = new Point(5,10);
	
	static {
		generate(100, 100);
	}
	
	public static void setSelectCell(int col, int row) {
	...
	public static void paint(Graphics g) {
		...
		for (int row = firstRow; row <= lastRow; row++) {
			for (int col = firstCol; col <= lastCol; col++) {
				...
				if(col==selectedCell.x && row==selectedCell.y) {
					g.setColor(Color.GREEN);
					g.fillRect(Camera.getScreenX(col * Block.SIZE),Camera.getScreenY(row * Block.SIZE), Block.SIZE, Block.SIZE);
				}
			}
		}
	}
